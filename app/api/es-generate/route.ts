...
export async function POST(req: Request) {
  try {
    const { question, charLimit, targetCompany, selfAnalysisData, companyData, selectedGakuchika } = await req.json()
...
## 出力形式（必ずJSON形式で返してください）:
{
  "es": "作成したES本文",
  "charCount": 文字数（数値）,
  "explanation": "この構成にした理由（STAR法のどの部分をどう使ったか）",
  "tips": "この ESをさらに良くするための具体的なアドバイス"
}

重要なルール:
- STAR法（状況→課題→行動→結果）に基づいて構成する
